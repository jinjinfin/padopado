...
export function openCapture({
  ...
  const retroMode = Boolean(presetRetroPeriod);
  modalEl.innerHTML = renderModal({ presetType, presetRetroPeriod, retroMode, referenceEntries });
  modalEl.classList.add('open');
  wireModal({ presetType, presetRetroPeriod, retroMode, prefillContent });
}
...
function renderModal({ presetType, presetRetroPeriod, retroMode, referenceEntries }) {
  ...
  return `
    <div class="modal-sheet" role="dialog" aria-label="새 기록">
      ...
      <div class="modal-header">
        <h2>${retroMode ? `🪞 ${retroLabel(presetRetroPeriod)} 작성` : '새로운 기록'}</h2>
      ...
      <button type="button" class="btn primary" id="capture-save">저장하기</button>
    </div>
  `;
}

function wireModal({ presetType, presetRetroPeriod, retroMode, prefillContent }) {
  ...
  $('#capture-save').addEventListener('click', async () => {
    ...
    const kind = collectionKindFor(selectedType);
    ...
    await entries.addEntry({
      ...
      tags,
      collectionId,
      retroPeriod: selectedType === 'retro' ? selectedRetro : null,
    });
